...
export function extractParamsFromPage(htmlContent: string): string | null {
  ...
  debugLog("📊 DEBUG: HTML content length:", htmlContent.length, "characters");
  ...
  try {
    ...
    try {
      ...
      try {
        ...
        // @ts-ignore
        debugLog("❌ DEBUG: Failed to parse ytInitialData:", jsonError.message);
        return null;
      }
    }
...
export function parseVideoPageWithFallbacks(
  ...
  debugLog("🔍 DEBUG: Params found on video page:", pageParams || "NOT FOUND");

  if (pageParams) {
    debugLog("📏 DEBUG: Page params length:", pageParams.length, "characters");
    debugLog(
